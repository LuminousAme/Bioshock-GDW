...
vec3 PhysicsBody::m_gravityAcceleration = vec3(0.f, 0.f, 0.f);
...
PhysicsBody::PhysicsBody(b2Body * body, float radius, vec2 centerOffset, bool isDynamic)
{
	...
	//creates the acutal fixture (aka shap, mass, etc.)
...
PhysicsBody::PhysicsBody(b2Body * body, float width, float height, vec2 centerOffSet, bool isDynamic)
{
	...
	//Creates the acutal fixtur (aka shape, mass, etc.)
	b2FixtureDef tempFixture; 
	tempFixture.shape = &tempShape;
	tempFixture.density = 1.f;
	tempFixture.friction = 0.3f;
...
void PhysicsBody::InitBody()
{
	...
	//Enable slot 0
	glBindVertexArray(0);	//Vertices

	//VBO positions
	m_vboPos = VertexManager::GetPlaneVertVBO(); 

	//push away the warnings
#pragma warning(push)
#pragma warning(disable : 4312)
	//bind position bugger
...
unsigned int PhysicsBody::GetCollidedID() const
{
	return m_collideID;
}
...
void PhysicsBody::SetAcceleration(vec3 acel)
{
	m_acceleration = acel; 
}
...
void PhysicsBody::SetTopRigth(vec2 TR)
{
	m_topRight = TR; 
}
