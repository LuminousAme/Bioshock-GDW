...
void PhysicsSystem::Update(entt::registry * reg, b2World & world)
{
	...
	{
		//Grabs reference to each component with view 
...
{
	//Timestep is constnat, regardless of deltatime
...
{
	if (group1.first.GetCollidedID() & group2.first.GetBodyID())
	{
		//Perform bounding box checks
		//Are the x-axises colliding
		...
		//If both axises are overlapping, it means the bodies are colliding
...
void PhysicsSystem::Draw(entt::registry * reg)
{
	...
	if (PhysicsBody::GetDraw())
	{
		for (auto entity : view)
		{
			...
			//Temporary transform so we can actually draw the bodis 
			Transform temp = trans; 
			temp.SetScale(vec3(physBody.GetWidth(), physBody.GetHeight(), 1.f)); 
			//Sets the position so the center offset is still relavent 
			temp.SetPosition(temp.GetPosition() + vec3(physBody.GetCenterOffset().x, physBody.GetCenterOffset().y, 0.f)); 
			//Puts the temporary transform for the physics body at the top z-;ayer
